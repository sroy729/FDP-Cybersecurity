...
struct hammeringDataStruct data_for_hammering;
...
void profile_singlesided(struct ProfileCtx *c)
{
	...
	while (hamqueue_ready(q, c->bm, !c->incomplete)) {
		...
		assert(!r);
		if (c->attack_check_fn && // Print the aggressor
		...
		for (int inv = 0; inv <= 0; inv++) {
			...
			data_for_hammering.va1 = ae[0].virtp;
			data_for_hammering.va2 = ae[1].virtp;
			data_for_hammering.c = c;
			// printf("Hammering Row");
